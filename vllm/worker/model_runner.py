...
class ModelRunner(GPUModelRunnerBase[ModelInputForGPUWithSamplingMetadata]):
    ...
    def execute_model(
        ...
        **kwargs,
        ...
        # Extract final hidden states for logits computation if needed
        if isinstance(hidden_or_intermediate_states, IntermediateTensors):
            if "final_hidden_states" in hidden_or_intermediate_states.tensors:
                final_hidden_states = hidden_or_intermediate_states.tensors["final_hidden_states"]
            else:
                # This shouldn't happen in normal flow, but fallback
                final_hidden_states = hidden_or_intermediate_states
        else:
            final_hidden_states = hidden_or_intermediate_states
            
        logits = self.model.compute_logits(final_hidden_states,
